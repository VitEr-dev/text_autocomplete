...
def preprocess_data(tweets_df, save_path='./data/'):
    ...
    clean_df['input_ids'] = clean_df['text'].apply(
        lambda x: tokenizer.encode(str(x), add_special_tokens=True) if not pd.isna(x) else []
    )
...
def create_sequence_samples(input_ids_list, sequence_length, sep_token_id=102, unk_token_id=100):
    """Создание обучающих последовательностей"""
    X_sequences = []
    y_sequences = []
    
    def is_valid_target(token_id):
        return token_id not in (sep_token_id, unk_token_id)
    ...
    for sequence in input_ids_list:
        ...
        for i in range(len(sequence) - sequence_length):
            ...
            y_seq = sequence[i + sequence_length]
            
            if not is_valid_target(y_seq):
                continue
